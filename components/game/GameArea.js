...
import { Image, Button, Stylesheet, PanResponder, Dimensions, StyleSheet, Text, Animated, View } from 'react-native';
...
class GameArea extends React.Component {
  ...
  componentWillMount() {
    this.setState({
      ...
      function () {
        _this = this;
        getPlayerHand(_this.state.gameId, _this.state.playerId,
          function(data) {
            _this.setState({ hand: data.hand, discard: [data.discard] });
          }
        );
        runCheckDiscard();
      }
  ...
  dropCardToDiscard(discardCard, callback) {
    ...
    if (_this.state.activeTurn === _this.state.playerTurnNum && _this.state.phase2) {
      ...
      _this.state.hand.splice(index, 1);
      ...
      discardPush(_this.state.gameId, _this.state.playerId, discardCard._id,
        function ( data ) { return data; } /**TODO Kill this??**/
      );
    }
    callback();
  }
  ...
  pickUpDiscard(card, handPositionVar, disOrDraw) {
    ...
    if (_this.state.activeTurn === _this.state.playerTurnNum && _this.state.phase1) {
      if (disOrDraw) {
        ...
        pickDiscard(_this.state.gameId, _this.state.playerId,
          function(data) {
            _this.setState({discard: [{'pictureId': 0}], phase1: false, phase2: true });
          }
        );
      } else {
        pickDraw(_this.state.gameId, _this.state.playerId,
          function(data) {
            _this.state.hand.splice(handPositionVar, 0, data);
            _this.setState({ hand: _this.state.hand, phase1: false, phase2: true });
          }
        );
      }
    }
  ...
  const styles = StyleSheet.create({
    ...
    navigationBar: {
      right: viewWidth * 0.4,
      paddingTop: viewHeight * (10 / 568)
    },
    container: {
      flex: 1,
      height: viewHeight * (80 / 568),
      width: viewWidth
    },
    bannerText: {
      color: 'white',
    ...
    winnerImage: {
      left: viewWidth * 0.1,
      marginTop: viewHeight * (50 / 568),
      height: viewHeight * (250 / 568),
      width: viewWidth * (250 / 320),
      zIndex: 2
    }
  });
